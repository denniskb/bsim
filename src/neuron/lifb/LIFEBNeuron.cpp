...
LIFEBNeuron::LIFEBNeuron(real v_init, real v_rest, real v_reset, real cm, real tau_m, real tau_refrac, real tau_syn_E, real tau_syn_I, real v_thresh, real i_offset)
...
LIFEBNeuron::LIFEBNeuron(const LIFEBNeuron &neuron) : NeuronBase()
{
	this->_vm = neuron._vm;
	this->_CI = neuron._CI;
	this->_CE = neuron._CE;
...
LIFEBNeuron::~LIFEBNeuron()
{
}

int LIFEBNeuron::recv(real I)
{
	if ( I >= 0 ) {
		_i_syn_E += I;
	} else {
...
Type LIFEBNeuron::getType()
{
	return type;
}

int LIFEBNeuron::reset(SimInfo &info)
{
	real dt = info.dt;
...
int LIFEBNeuron::update(SimInfo &info)
{
	fired = false;
...
void LIFEBNeuron::monitor(SimInfo &info)
{
}

size_t LIFEBNeuron::getSize()
{
	return sizeof(GLIFEBNeurons);
}

int LIFEBNeuron::getData(void *data)
{
...
int LIFEBNeuron::hardCopy(void * data, int idx, int base)
{
	GLIFEBNeurons *p = (GLIFEBNeurons *) data;
	setID(idx+base);
	p->p_vm[idx] = _vm;
	p->p_CI[idx] = _CI;
	p->p_CE[idx] = _CE;
