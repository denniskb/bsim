...
void benchfun( model m, gpu g, int nneuron, double pconnect, int delay, double pfire );
    ...
    std::string bench = fire::arg( "--bench" ) )
{
	...
	benchfun( str2model( model ), str2gpu( gpu ), nneuron, pc, d, pf );
...
void benchfun( model m, gpu g, int nneuron, double pconnect, int delay, double pfire )
{
	...
	try {
		Network c;
		switch(m) {
			case model::vogels: make_vogels(c, nneuron); break;
			case model::brunel: make_brunel(c, nneuron); break;
			case model::synth: make_synth(c, nneuron, pfire, pconnect, delay); break;
			default: throw std::runtime_error("not implemented");
		}
		
		std::unique_ptr<SimulatorBase> sg;
		switch(g) {
			case gpu::single: sg.reset(new SGSim(&c, 0.0001f)); break;
			case gpu::multi:  sg.reset(new MGSim(&c, 0.0001f)); break;
		}
